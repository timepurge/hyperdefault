...
var CONFIG = { TYPES: { o: "object", a: "array" } };var ruleparse = function ruleparse(rulekey, ruleobj) {
  var ruleval = ruleobj[rulekey];if (typeof ruleval === "string") {
    if (ruleval.match(/\$\{([\s]*[^;\s\{]+[\s]*)\}/g)) {
      var link = ruleval.match(/[\w\.:]+/)[0].split(":");if (link.length == 2) {
        return { linked_key: link[0], linked_type: link[1], isrule: true };
      }
    }
  }return { value: ruleval, isrule: false };
  ...
  if ((typeof target === "undefined" ? "undefined" : _typeof(target)) === CONFIG.TYPES.o) {
    for (var key in ruleobj) {
      var _propdef = ruleparse(key, ruleobj);if (_propdef.isrule) {
        ruleextend(target, _propdef, rule, key);
      } else {
        if (!(key in target)) {
          var _ruleval = _propdef.value;if ((typeof _ruleval === "undefined" ? "undefined" : _typeof(_ruleval)) === CONFIG.TYPES.o) {
            if (Array.isArray(_ruleval)) {
              target[key] = _ruleval;
            } else {
              target[key] = {};extend(target[key], _ruleval);
            }
          } else {
            target[key] = _ruleval;
          }
